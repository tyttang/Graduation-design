...
When /^I click the logout button$/ do
  on_page(LoginPage).logout_button_click
end
...
And /^I will see the popup '([^"]*)'$/ do |information1|
    on_page(LoginPage).logout_msg.should include information1
end

And /^I will select Yes and return login page$/ do
  on_page(LoginPage).logout_popup_yes
end

And /^I will select No and close the popup$/ do
  on_page(LoginPage).logout_popup_no
end

Then /^I can see the logout button existence$/ do
  on_page(LoginPage).logout_button_existence
end
...
Then /^I will see the notification '([^"]*)'$/ do |information2|
  on_page(LoginPage).login_notification_text.should include information2
end
